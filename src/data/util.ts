...
export const getProjects = async (locale: Locale) => {
  ...
  try {
    ...
    for (const path in projectModules) {
      ...
      // Try to find the markdown module
      if (markdownModules[mdPath]) {
        const mdModule = markdownModules[mdPath] as any
        project.default.Content = mdModule.Content
      } else {
        console.warn(
          `Markdown file not found for project ${project.default.slug} and locale ${locale}`,
          mdPath,
        )
      }
    }

    projects = Object.values(projectModules) as Project[]
...
export const getTechnologies = (
  ...
  const frameworksModules = import.meta.glob('./tech/frameworks/*.js', {
    eager: true,
  })
